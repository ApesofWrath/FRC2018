...
const double MAX_THEORETICAL_VELOCITY_E = (free_speed_e / G_e) * 60 * PULLEY_RADIUS * 2.0 * PI;
...
const double MID_ANGLE_E = 1.0; //TEST
...
std::vector<std::vector<double> > K_e = { { 471.817, 11.17 }, { 471.817, 11.17 } }; //controller matrix that is calculated in the Python simulation
...
Elevator::Elevator(PowerDistributionPanel *pdp, MotionProfiler *elevator_profiler_) {

	elevator_profiler = elevator_profiler_;

	elevator_profiler->SetMaxAcc(2.0);
...
void Elevator::Move(std::vector<std::vector<double> > ref_elevator) {
	...
	u_e = (K_e[0][0] * error_e[0][0]) + (K_e[0][1] * error_e[1][0]) + Kv_e * goal_vel_e; // for this system the second row of the K matrix is a copy and does not matter. //
...
void Elevator::ElevatorWrapper(Elevator *el) {
	...
	while (true) {
		while (frc::RobotState::IsEnabled()) {
			...
			//	std::cout << "pos: " << profile_elevator.at(0).at(0) << "  " << "vel: " << profile_elevator.at(1).at(0) << "   " << "acc: " << profile_elevator.at(2).at(0) << std::endl;
