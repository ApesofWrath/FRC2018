...
//upon re-enable, will resume in the state it was in
	...
	switch (state) {
		...
		case INIT_STATE:
		  SmartDashboard::PutString("STATE", "PRE-INIT");
		if (elevator->is_elevator_init && intake->is_init_intake) { //nothing will happen if haven't initialized arm/elev
