//TODO: change 'elevator' -> 'outerelevator'

#include <OuterElevator.h>
#include "ctre/Phoenix.h"
#include <WPILib.h>

OuterElevator::OuterElevator(ElevatorMotionProfiler *elevator_profiler_) {
...
void Elevator::InitializeElevator() {
	...
	if (!is_elevator_init) { //don't see hall effect anymore
...
void Elevator::Move() {
	...
	//	SmartDashboard::PutNumber("Goal Pos", goal_pos);
		...
		v_bat_e = 12.0; //battery voltage

		if (elevator_profiler->GetFinalGoalElevator() //moving to a ref lower than where elevator started
				< elevator_profiler->GetInitPosElevator()) { //can't just look at the next goal in case we get ahead of the profiler
			K_e = K_down_e;
